"""
Add voiceover to the Remotion-rendered ground-up construction loan video --
same timestamp-driven approach as the JPMorgan/DSCR videos: generate ONE
continuous take at natural pace via ElevenLabs' with-timestamps endpoint,
find each line's real start time in that audio, then print the Remotion
scene durations that make the VIDEO match the VOICE (video timed to voice,
not guessed).

Run from newsletter_video_pipeline/remotion/:  python voiceover_construction.py
Then apply the printed durationInFrames values to ConstructionVideo.tsx and
the printed TOTAL_S to Root.tsx, re-render, and this script will mux the
exact same audio it measured onto the fresh render.
"""
import base64
import json
import os
...
SWIPE = 0.35  # matches SWIPE in ConstructionVideo.tsx
FPS = 30
TAIL_PAD = 2.0

# One continuous script, in on-screen order.
LINES = [
    "You don't need experience to get a construction loan.",
    "The lender isn't worried about the house. It's worried about everything before it exists.",
    "They're evaluating your contractor, your schedule, your budget, and your exit.",
    "The real costs go beyond the build itself.",
    "An understated allowance doesn't save money. It only postpones the invoice.",
    "Funds release in stages: work done, draw requested, inspection, then funds released.",
    "Loan to cost tells you the percentage. It doesn't tell you what you need this week.",
    "Build to sell needs real comps. Build to rent needs supportable income.",
    "Perfect weather, zero change orders, a record sale price. That's not a plan.",
    "The best first deal has a proven product type and few open questions.",
    "Plan beats experience. Budget beyond the build. Know your exit.",
...
def main():
    ...
    voice_dir = base / "_voiceover_construction"
    ...
    print("\nScene durationInFrames to use in ConstructionVideo.tsx (video timed to this voice take):")
    ...
    video_path = base / "out" / "construction.mp4"
    final_path = base / "out" / "construction_voice.mp4"
    if video_path.exists():
        ...
        if abs(cur_video_dur - total_s) < 0.5:
            ...
            print(f"\nNOTE: out/construction.mp4 is {cur_video_dur:.2f}s but this voice take needs {total_s:.2f}s --")
            print("apply the durations above to ConstructionVideo.tsx/Root.tsx and re-render before muxing.")
